...
// src/ConfusablesParser.ts
import * as fs from "fs";
import * as path from "path";
import * as url from "url";
var __filename = url.fileURLToPath(import.meta.url);
var __dirname = path.dirname(__filename);
var ConfusablesParser = class {
  ...
  constructor() {
    ...
    for (const line of lines) {
      ...
      const sourceChars = sourceHex.map((hex) => String.fromCodePoint(parseInt(hex, 16)));
      const targetChars = targetHex.map((hex) => String.fromCodePoint(parseInt(hex, 16)));
      const isTargetAscii = targetChars.every((char) => char.charCodeAt(0) <= 127);
      const isSourceOnlyAscii = sourceChars.every((char) => char.charCodeAt(0) <= 127);
...
// src/index.ts
var Homoglypher = class _Homoglypher {
  static parser = new ConfusablesParser();
  //
  ...
  constructor(o) {
    ...
    this.maxLen = Math.max(_Homoglypher.parser.getMaxSourceLength(), customMaxLen, 1);
  }
  //
  normalize(input, o) {
    let output = "";
...
// examples/tools/index.ts
var homoglypher = new Homoglypher();
function NormalizeAndVisualize(input) {
  const changes = {};
