...
const outputPath = path.join(__dirname, '..', 'src/ConfusablesParser', outputFilename);
...
https.get(fileURL, (res) => {
    if (res.statusCode !== 200) {
        console.error(`❌ Fetching Error, statusCode: ${res.statusCode}`);
        res.resume();
        return;
    }
    ...
    res.on('end', () => {
        ...
        console.log(`✅ File ${outputFilename} saved to ${outputPath}`);
    });
}).on('error', (err) => {
